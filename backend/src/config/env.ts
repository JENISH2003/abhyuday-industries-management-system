...
export const config = {
  ...
  SMTP_HOST: process.env.SMTP_HOST || (process.env.EMAIL_USER?.includes('gmail') ? 'smtp.gmail.com' : 'smtp.ethereal.email'),
  SMTP_PORT: parseInt(process.env.SMTP_PORT || (process.env.EMAIL_USER?.includes('gmail') ? '465' : '587'), 10),
